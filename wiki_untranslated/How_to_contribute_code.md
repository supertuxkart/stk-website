...
toc: true
---
_(Last verified to be up-to-date: 06/05/2026)_
...
* Try to minimize data dependencies. A more complex tree of interactions means code that is more difficult to understand and higher odds for bugs.
...
## SuperTuxKart's policy on AI-generated code

**Code generated by generative AI is not accepted in SuperTuxKart. Any code submission produced with this method will be rejected.**

There are several important reasons for this strict policy:
* The specialty of LLMs is to produce an output that looks convincing on the surface, without a deeper understanding of what is conveyed by the words or symbols present in the output. With code, this translates to **architectural issues and subtle bugs that lead to a maintenance nightmare**.
* Most of the work when implementing a feature or fixing a bug is not in writing the actual lines of code, it is in properly understanding what is going wrong or what form the solution should take, and in properly testing the new code to see if it works as intended and handles edge cases properly. **The commonly touted time savings essentially disappear once proper design and quality control is taken into account.**
* Use of generative AI pushes towards complacency. It leads to an atrophy of the very skills that are needed to be able to think about code design and to catch issues present in the generated code.
* **Reviewing code contributions takes time and effort.** With LLMs, it is easy to quickly generate a lot of low-quality code, which wastes the team's scarce time.
* **AI-generated code creates serious licensing concerns**, between the concerns of plagiarism from the LLM's training data and the fact that only the creative work of humans is copyrightable.

Please note that using AI to generate code comments, commit messages... is not accepted either.

There are a few limited usages of LLMs that are acceptable, chiefly:
* Helping to understand the structure of existing code. As SuperTuxKart's code is quite complex, it might be occasionally helpful to use a LLM to assist you in analyzing how an unfamiliar portion of the code works. In this case, it is still important to manually confirm what is going on, as the goal is to acquire genuine understanding.
...
The duration between the submission and the merge of the PR can vary from a few hours to several weeks depending on code complexity and the availability of the team's developers, although usually you will receive comments within a few days. PRs that are a few months old usually contain some element that make them unsuitable for direct merging in their current state.
